...
def build_commands(models, prune_ratios, quant_levels, modes, quick=False, trials=1, seed=42):
    cmds = []
    modes = set(modes)
    for model in models:
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=42, help="Base random seed (incremented per trial)")
    ...
    commands = build_commands(args.models, args.prune_ratios, args.quant_levels, args.modes, quick=args.quick, trials=args.trials, seed=args.seed)
    tasks = []
